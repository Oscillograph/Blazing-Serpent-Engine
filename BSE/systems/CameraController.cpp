	...
	void CameraController::OnUpdate(float time){
		...
		if (allowRotation){
			...
			if (Input::IsMouseButtonPressed((int)KeyCode::MouseButtonRight)) {
				...
				float Yaw = m_CameraRotation.y - delta.x;
				float Pitch = m_CameraRotation.x - delta.y;
				
				// rotate camera
				Rotate({m_CameraRotation.x, Yaw, m_CameraRotation.z});
				Rotate({Pitch, m_CameraRotation.y, m_CameraRotation.z});
				
				// move camera
				Yaw = glm::radians(-delta.x );
				Pitch = glm::radians(-delta.y);
				
				SetCameraPosition(glm::rotate(m_CameraPosition, Yaw,   {0.0f, 1.0f, 0.0f}));
				SetCameraPosition(glm::rotate(m_CameraPosition, Pitch, {1.0f, 0.0f, 0.0f}));
			} else {
