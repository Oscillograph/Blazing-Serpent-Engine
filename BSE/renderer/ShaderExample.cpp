...
namespace BSE {
	ShaderExample::ShaderExample(const std::string& vertexSrc, const std::string fragmentSrc){
		...
		if(isCompiled == GL_FALSE) {
			...
			BSE_CORE_ASSERT(false, "Vertex shader compilation failed.");
		...
		if (isCompiled == GL_FALSE) {
			...
			// Use the infoLog as you see fit.
			BSE_CORE_ERROR("{0}", infoLog.data());
			BSE_CORE_ASSERT(false, "Fragment shader compilation failed.");
	...
	void ShaderExample::Bind() const {
		glUseProgram(m_RendererId);
	}
	
	void ShaderExample::Unbind() const {
		glUseProgram(0);
	}
	
	void ShaderExample::UploadUniformMat4(std::string name, const glm::mat4& matrix){
